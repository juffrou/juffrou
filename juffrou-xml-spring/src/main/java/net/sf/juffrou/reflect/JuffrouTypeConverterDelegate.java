 ...
 * <p>
 * Works on a given {@link PropertyEditorRegistrySupport} instance. Used as a delegate by {@link BeanWrapperImpl} and
 ...
 */
class JuffrouTypeConverterDelegate {

	private static final Log logger = LogFactory.getLog(JuffrouTypeConverterDelegate.class);

	private final JuffrouSpringBeanWrapper propertyEditorRegistry;
	 ...
	 */
	public JuffrouTypeConverterDelegate(JuffrouSpringBeanWrapper propertyEditorRegistry) {
	 ...
	 */
	public JuffrouTypeConverterDelegate(JuffrouSpringBeanWrapper propertyEditorRegistry, Object targetObject) {
	...
	public <T> T convertIfNecessary(Object newValue, Class<T> requiredType, MethodParameter methodParam) throws IllegalArgumentException {
		...
		return convertIfNecessary(null, null, newValue, requiredType, (methodParam != null ? new TypeDescriptor(
				methodParam) : TypeDescriptor.valueOf(requiredType)));
	}
	...
	private Collection convertToTypedCollection(Collection original, String propertyName, Class requiredType, TypeDescriptor typeDescriptor) {
		...
		for (; it.hasNext(); i++) {
			...
			Object convertedElement = convertIfNecessary(indexedPropertyName, null, element, (elementType != null ? elementType.getType()
					: null), elementType);
			try {
	...
	private Map convertToTypedMap(Map original, String propertyName, Class requiredType, TypeDescriptor typeDescriptor) {
		...
		while (it.hasNext()) {
			...
			Object convertedKey = convertIfNecessary(keyedPropertyName, null, key, (keyType != null ? keyType.getType()
					: null), keyType);
			Object convertedValue = convertIfNecessary(keyedPropertyName, null, value, (valueType != null ? valueType.getType()
					: null), valueType);
			try {
